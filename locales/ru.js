module.exports = {
  Footer: {
    'Online money management FINEX': 'Домашняя бухгалтерия FINEX',
    Agreement: 'Пользовательское соглашение',
    'Privacy Policy': 'Политика конфиденциальности',
    'Terms of Use': 'Правила использования',
  },
  Home: {
    'Узнать больше': 'Узнать больше',
    'Начать пользоваться': 'Начать пользоваться',
  },
  HowItWork: {
    Описание: 'Описание',
    'Как работает FINEX': 'Как работает FINEX',
    'Остались вопросы?': 'Остались вопросы?',
    'Напишите нам': 'Напишите нам',
  },
  Jumbotron: {
    'Посмотреть как это работает': 'Посмотреть как это работает',
  },
  PriceCard: {
    'until cancelled': 'до отмены',
  },
  Pricing: {
    'FINEX Pricing': 'Подписка на FINEX',
    'Get started for free with 14 day trial period': 'Начните бесплатно с пробным периодом 14 дней',
  },
  Seo: {
    'FINEX | Budget Tracker & Planner | Personal Finance Management Online':
      'FINEX | Планирование бюджета | Управление личными финансами онлайн',
    'Online personal finance management tool FINEX helps you easily organize and track your spending, create a budget, and reach your financial goals_':
      'Онлайн-инструмент FINEX для управления личными финансами поможет вам легко организовывать и отслеживать свои расходы, составлять бюджет и достигать финансовых целей.',
    'personal finance, finance management, budgeting, financial goals, money management, cash flow management, debt management, expense tracking, financial planning':
      'управление финансами, составление бюджета, финансовые цели, управление деньгами, управление денежными потоками, управление долгом, отслеживание расходов, финансовое планирование',
  },
};
